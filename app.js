...
function applyROMContext(romData) {
  ...
  const sections = `<div class="kinem-scroll${regions.length > 1 ? ' kinem-multi' : ''}">` +
    regions.map(r => {
      ...
      return `<div class="kinem-item" style="border:1px solid rgba(79,156,249,0.2);border-radius:6px;padding:6px 8px">` +
        `<div style="font-size:11px;color:#4f9cf9;margin-bottom:3px">${r.label}</div>` +
