...
- **`track()` is telemetry, never control flow.** It runs *after* `modeFor` has resolved, writes one row per request to Analytics Engine (dataset `physiq_usage`, shared with the copilot worker — `blob1` is `'report'` here), and is wrapped in try/catch with an optional binding. Nothing in the request path may branch on it. The hub's `scripts/usage-report.js` queries the dataset.
